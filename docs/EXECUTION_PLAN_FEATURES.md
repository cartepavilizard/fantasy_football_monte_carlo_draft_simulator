...
| H10 | **Wire ffanalytics in as a permanent push source.** One line — `register_push_source("ffanalytics", parse_udk_rows)` in `backend/data_sources/service.py` — plus a test that the generic `POST /rankings/push/ffanalytics` route round-trips the R producer's CSV. H1 built the registry for exactly this and H4 deliberately spelled the CSV headers to match `udk.py`'s `COLUMN_ALIASES`, so no parser work is needed. | [Sonnet] | **Open — created by H5 (2026-07-31), gated on the owner's answer to question (3) in `docs/specs/H5_ADOPTION_GATE.md`.** H5 measured the arm and says adopt: 1870/1902 rows resolve, +61 projected players, best-behaved rescale factors of any source, and 465 players gain a third projection opinion. This row is only the *permanent wiring* — H5 already proved the blend path works end-to-end by building the batch in memory. **Do not start it before the owner answers**, and note the producer needs `R_LIBS_USER=~/R/library` under WSL or `library(ffanalytics)` fails. | 0.25d | **H5 measured — owner sign-off** |
