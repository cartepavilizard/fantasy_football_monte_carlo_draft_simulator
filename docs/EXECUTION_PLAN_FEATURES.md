...
done (backend core + frontend switcher); B5 core done with its cheap half
speced (see task notes). Suite at 220 passed / 0 failed. Remaining for exit:
B5 panel CRUD + frontend + a real Routine, then a live sync test against the
three real leagues (`ESPN_S2`/`ESPN_SWID`/`ESPN_LEAGUE_IDS` are already set)
once run against an environment with a reachable MongoDB.
