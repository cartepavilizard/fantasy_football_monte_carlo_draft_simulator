...
| H12 | **Give `tier_confidence` a consumer: an uncertainty-aware near-tie margin.** `value_over_wait.TIE_MARGIN_POINTS` is a flat 5.0. When the two positions in contention are led by low-confidence players, the gap between them is less trustworthy and the margin should widen. | [Opus 5] | **Open — created by H7 (2026-08-02). This CHANGES LIVE DRAFT RECOMMENDATIONS, so it is a measured-gate row, not a code row.** Follow the H5 pattern exactly: implement the widening behind a config knob that **defaults to the current flat behaviour**, then diff `value_over_wait` verdicts seed-paired across the live Mahomes leagues and rounds 3/5/7/9/11 the way `h5_adoption_gate.py` and H5's writeup do, and only then decide the default. H5 measured that verdicts are near-immovable except at genuine near-ties — which is precisely where this row intervenes, so expect the diff to be small, concentrated, and exactly at the round-7-style toss-ups. **Mandatory scope limit from H7's measurement:** apply it to skill positions ONLY. The expert-dispersion channel is meaningless for kickers, where a huge `expert_rank_std` (Ryan Fitzgerald 90.2) sits beside a tiny `projection_spread` (5.71) because experts are indifferent, not uncertain. `SCARCITY_POSITIONS` is already qb/rb/wr/te, so scoping costs nothing. | 0.5d | **H7 (done)** |
