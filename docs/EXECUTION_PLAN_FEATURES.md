...
notes). Suite at 280 passed / 0 failed. Remaining for Sonnet sessions:
C3, C5, C8, C9, C4's ingestion transform, C6's UI wiring, C7's flagging
+ UI. Do C4-ingest before C3/C5 only if usage data is wanted in their
views; otherwise C3 → C7-flags → C9 → C4-ingest → C6-UI → C5 → C8 is a
sensible order (C3 unblocks streaming before week 1).
...
| C3 | **K/DST streaming recommendations**: weekly rank of available kickers/defenses by matchup, from C2's data + B1's free-agent list. | [CHEAP] | Ready to build: filter latest `FreeAgentSnapshot` to K/DST, join `defense_position_strength()` on each player's week opponent (`opponent_map()` in `models/matchup_strength.py`), rank by ESPN weekly projection × tilt (`matchup_adjusted`), tie-break by multiplier. New GET in `inseason_api.py` (add to both cached-only enforcement tests) + a panel in the in-season UI. |
| C4 | **Snap count & target share trends + usage-shift alerts**: ingest weekly usage data; detect meaningful shifts (rising backup, shrinking role) and raise alerts through B5. Process-over-results: alerts framed on volume/opportunity, never one-week points. | [SPLIT] | **Core done (2026-07-11).** Source decided: **nflverse CSV releases** (snap counts + weekly player stats incl. `target_share`). Signal contract in `models/usage_shifts.py`: current week vs mean of last 2–4 prior weeks; thresholds 0.12 snap / 0.07 target share; floors 0.15/0.10 drop roster churn; min 2 prior weeks → first alert week 3; both directions. Alerts dedupe via `ensure_notification`, restricted to rostered/free-agent players, copy volume-only. `GET /inseason/usage_shifts` serves the trends read. **Cheap half:** the ingestion transform — full spec (URLs, columns, merge, replace semantics, scheduler wiring behind `USAGE_INGEST_ENABLED`) at the bottom of `models/usage_shifts.py`. |
| C5 | **Playoff schedule analysis**: weeks 14–16 strength of schedule per position, per team; feeds roster and trade decisions. Needed by early Nov, can land later in Sep. | [CHEAP] | Ready to build: for each NFL team, weeks 14–16 opponents from `ProGame`; score each (position, team) by summing `defense_position_strength()` multipliers of those opponents (through_week param exists for windows); report rank per position + per fantasy roster (join current rosters). New GET in `inseason_api.py` + report view; confidence carries over from C2 (early-season = mostly neutral, say so). |
| C6 | **Lineup-locking strategy**: for early-game players (Thu/Wed opener), suggest flex/bench placement that locks early and preserves Sunday flexibility. | [SPLIT] | **Core done (2026-07-11) — both rules defined and implemented in `models/lineup.py`.** Rule 1 (free, always applied): among equal-total lineups, a second DP puts early-locking players in restrictive slots and late-locking players in flex-type slots, so flexible slots stay unlocked longest; Wednesday opener needs no special case. Rule 2 (advice only): any starter locking ≥36h before final lock with a later-kicking bench alternative within 1.0 pt gets a `lock_advice` entry quoting the exact cost (margin ≈ option value). **Cheap half:** render `lock_advice` + kickoff times in the lineup UI (badge early-locking starters, advice cards with cost). Tests: `test_lineup_locking.py`. |
| C7 | **Handcuff strategy**: maintain a starter→direct-backup map; flag when a key starter's handcuff is available and worth rostering. | [SPLIT] | **Core done (2026-07-11).** Sourcing decided: **curated seed table** over depth-chart inference (inference is weakest in Sept when handcuff value peaks; committees encoded by omission; rationale in `models/handcuffs.py`). ~24 RB pairs seeded from end-of-2025 depth charts — **needs a human review pass in Aug 2026**. Additive-only seeding, manual-marked edits, soft deletes (re-seed can't resurrect a committee call). CRUD live: `GET/POST /inseason/handcuffs`, `POST /inseason/handcuffs/seed`, `DELETE /inseason/handcuffs/{starter}`. **Cheap half:** flagging logic + UI — full spec (join vs rosters/free agents, priority=high on starter injury, notification only then, C9 call site) at the bottom of `models/handcuffs.py`. Tests: `test_handcuffs.py`. |
| C8 | **Process-over-results framing**: recommendation copy across the module cites volume/opportunity, flags single-game variance explicitly ("1 catch on 9 targets" ≠ "bad game"). | [CHEAP] | Convention already established at the sources: C4's alert copy (`_shift_copy` in `models/usage_shifts.py`) and C1's review notification never cite points. Remaining: apply it across the in-season UI (usage trends view showing share-vs-baseline, single-game variance flag when targets high but catches low from `PlayerWeekUsage`, tooltip language) and to C3/C5/C7 output copy as they land. |
| C9 | **Homer check (in-season scope)**: reuse A6's neutral comparison whenever a Seahawks player is suggested as a waiver add or trade piece. | [CHEAP] | Unchanged: reuse `homer_check()` (`models/homer.py`) as-is at the new call sites — C3's streaming list and C7's handcuff flags first (pass the same-position free-agent pool; `pick_number=None`). Needs a light adapter from `FreeAgentEntry` to the player shape `homer_check` reads (name/nfl_team/points/consensus_rank/adp/tier/tag — projection from `projected_points`). |
