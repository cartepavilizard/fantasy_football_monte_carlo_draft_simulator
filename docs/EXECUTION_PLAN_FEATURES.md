...
| H11 | **Per-league tier cutoffs — kill the `ROSTER_SIZE` ambiguity.** `PositionTiers` derives every `position_tier` boundary from the module-global `config.ROSTER_SIZE`, read from the environment at import. Derive them from `len(league.teams)` at sync time instead. | [Sonnet] | **Open — created by H5 (2026-07-31). BLOCKS the H5 sync: do this first, or the sync bakes the wrong cutoffs into every tier permanently.** The name means two different things and that is the bug: `config.ROSTER_SIZE` (global, 12) is a **team count** (`position.py`'s comment says "14 in a 14-team league"; `qb1 = QB_SIZE * ROSTER_SIZE` is "1 starting QB × N teams"), while `League.roster_size` (per league, stored **15** on both leagues, equal to their `round_size`) means **players per team** and is read by **nobody**. So the quantity that should vary per league already does, in a field the tier code ignores — Mahomes is 10 teams, Skunkweed is 12, and one global cannot serve both. The drift H5 found (16 → 12) is almost certainly someone reading the name literally: 16 is close to a 15-man roster and nowhere near either team count. **Approach:** give `PositionTiers` a `for_team_count(n)` constructor and thread the league's team count into `Players`' tier-assignment validator (an optional field defaulting to `ROSTER_SIZE` keeps every existing caller — the CSV upload path, historical uploads, tests — working). **Leave `League.roster_size` in place**; renaming a persisted field needs a Mongo migration and buys nothing, so document its real meaning instead. **This row is a recalibration and must be measured like one:** changing cutoffs re-keys `position_tier_distributions` membership and therefore `randomized_points()`. `backend/scripts/h5_adoption_gate.py` clause 2 currently asserts ≥90 tier reassignments under the global; that number will move and the clause must be **re-measured and updated with its reasoning**, never weakened or deleted to get green. No Mongo writes — this changes what a *future* sync produces. | 0.5d | **— (do before the H5 sync)** |
...
8. **`ROSTER_SIZE` drift, found by H5 (2026-07-31) — decide before any
   ...
   than from an env-read global — now tracked as **H11**, which blocks
   the sync. Reproduce with
