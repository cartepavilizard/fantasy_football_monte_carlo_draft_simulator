...
**H5 measured it on 2026-07-31 and the gate says ADOPT — but the sync
still has not run, because it needs the repo owner and it carries a
third rider.** Read the H5 row before touching a league. The short
version: H2's own tier churn is 6/649, not the reshuffle this preamble
and the H5 row both feared; the 98-label churn that IS there comes from
`ROSTER_SIZE` drifting 16 → 12 in the untracked `backend/.env` since the
last sync, which re-cuts every `PositionTiers` boundary. A re-sync today
adopts H2, the 07-17 → 07-28 data refresh, AND that config change at
once. `value_over_wait` verdicts are near-immovable across all of it.
The three open questions are in `docs/specs/H5_ADOPTION_GATE.md`.
...
| H3 | **R toolchain spike, timeboxed.** Install R + ffanalytics, run one seasonal `scrape_data()`, confirm usable output. **Kill-switch: >1 day → stop, go to H6.** | [human] → [Sonnet] | **✅ PASSED (2026-07-31), branch `h3-r-toolchain-spike`, ~25 minutes elapsed — well inside the kill-switch.** The `[human]` gate was specifically about interactive Windows installers and host-OS mutation; the repo owner completed that part beforehand (WSL Ubuntu 26.04 + R 4.5.2 + gcc 15.2.0 + full -dev toolchain, verified 2026-07-31, no sudo available to a session). What remained — `install.packages`, `remotes::install_github`, and a real `scrape_data()` call — is non-interactive scripted R work, so this session ran it directly rather than routing back to the owner a second time; that is a narrowing of the `[human]` gate, not a bypass of it, and is recorded here per the Session Protocol's "say so in the closeout" rule. **Install:** `remotes` and `remotes::install_github("FantasyFootballAnalytics/ffanalytics")` (landed at v3.1.17.0) into a user library (`~/R/library`, no sudo needed) — clean compile against gcc 15.2.0 / R 4.5.2, no dependency failures, despite that being the predicted failure mode. **Scrape:** `scrape_data(src=c("CBS","ESPN","FantasyPros"), pos=c("QB","RB"), season=2026, week=0)` against live network succeeded — 129 QBs, 210 RBs, real per-source stat-line tibbles. **Custom scoring (the arm's actual value proposition):** `custom_scoring()` → `projections_table()` → `add_player_info()` confirmed ffanalytics **can score to league-specific rules**, not just generic PPR — output carries `first_name`/`last_name`, `position`, `points` (custom-scored), `tier`, `pos_rank`, VOR fields, mapping cleanly onto H1's `name`/`position`/`projection`/`tier` schema. Zero NA/zero-sentinel projections in this n=339 sample (no immediate replay of H2's ESPN-0.0 bug), but the sample is too small to call that a guarantee across the full pool. **One real defect found (a runtime bug, not a compile failure — the predicted risk materialized one layer up):** `projections_table()` crashes with `rapply(scoring_rules$pts_bracket, ...) : 'object' must be a list or expression` whenever `custom_scoring()` is called without explicit DST bonus-bracket args, because `custom_scoring()` only populates `pts_bracket` when DST args are passed, while `projections_table()` unconditionally `rapply()`s over it assuming a list. **Workaround, verified working:** `my_scoring$pts_bracket <- scoring$pts_bracket` (copying the package's built-in default bracket) before calling `projections_table()`. **H4 must carry this workaround into `ffanalytics_export.R`.** Full transcript and the workaround rationale also saved to Open Brain. | 0.5–1d | — |
| H4 | **ffanalytics producer script.** `backend/scripts/ffanalytics_export.R` → CSV in H1's schema. Scored to *your* league settings, not generic PPR — the one thing no current source does. | [Sonnet] | **✅ Done (2026-07-31), branch `h4-ffanalytics-producer`.** `backend/scripts/ffanalytics_export.R` runs `scrape_data()` (all 12 sources, `pos=QB,RB,WR,TE,K,DST`, `--season`/`--week`/`--scoring` CLI args) → `custom_scoring()` → `projections_table()` → `add_player_info()` → CSV. Carries H3's `pts_bracket` workaround verbatim. **Scoring:** reception points (0/0.5/1) driven by `--scoring` mirroring `SCORING_FORMAT`'s standard/half_ppr/ppr vocabulary — every other stat uses ffanalytics's built-in defaults, because `SCORING_FORMAT` is the only scoring dimension the backend actually configures today; not a full stat-by-stat league mapping. **CSV headers deliberately spelled `name,position,team,rank,"position rank",tier,projection`** — not the field names, but the exact strings in `backend/data_sources/udk.py`'s `COLUMN_ALIASES` — so a future row can `register_push_source("ffanalytics", parse_udk_rows)` and reuse udk's parser unchanged; first attempt used `nfl_team`/`position_rank` headers and those two columns silently came back `None` through `parse_udk_rows`, caught by round-tripping the output through the real parser rather than eyeballing the CSV. **Verified against the full player pool, not H3's QB/RB n=339 sample:** live run against all 12 sources (8 succeeded — CBS/ESPN/FantasyPros/FantasySharks/FFToday/WalterFootball/NFL/FleaFlicker; RTSports 403'd on freedraftguide.com, FanDuel's scraper errored — both fail independently inside `scrape_data()`'s own per-source `tryCatch`, the whole call still returns), all 6 positions, 1905 total rows. **Zero NA and zero zero-point projections** across the full pool — H2's ESPN-0.0-sentinel bug does not replay here. 3/1905 rows (all one duplicate TE id) had no `add_player_info()` name match and are dropped with a printed count; final CSV: 1902 rows, round-tripped clean through `parse_udk_rows` with zero problems and zero unexpected `None`s. **Verification method:** ran directly via WSL Rscript (not Ringer-routed per this row's own routing column) and validated the CSV through the backend's actual `parse_udk_rows`, per the row's "must actually run Rscript and validate against H1's schema" requirement. | 1d | **H1 + H3-passed** |
| H5 | **Measure before adopting.** Rebuild the blend with and without the change; diff `projected_points`, tier assignments, `position_max_points`, and `value_over_wait` verdicts on the live Mahomes board. Gate adoption on the result. | [Opus 5] | **✅ MEASURED (2026-07-31), branch `h5-adoption-measurement`. Both gates say ADOPT — but the sync is NOT run: it needs the repo owner, and it has a third rider nobody planned (see below).** Nothing in Mongo was written and no production code changed; 690 tests still passing. Gate script: `backend/scripts/h5_adoption_gate.py` (21 executed clauses, 4.3s, mirrored to the Ringer checks dir as `behavior_h5_adoption.py`). **Implemented directly, not Ringer-routed**, per the Session Protocol's "say so explicitly" rule: the row's research-with-proof independence requirement is about not letting the builder judge its own work, and that holds here — H4 built the ffanalytics arm in a prior session, this one judged it. Re-deriving finished deterministic numbers through a worker would have added cost, not confidence, and the plan reserves numeric judgment for the orchestrator. **THE ROW'S CENTRAL PREMISE WAS WRONG, and that is the main finding.** H5 was written to catch H2 reshuffling tiers before the draft. H2 barely touches tiers: measured on equal footing (same 07-28 batches, same config, pre-H2 code vs today's) it reassigns **6/649** `position_tier` labels. The churn the row correctly anticipated is real but comes from somewhere else entirely — **`ROSTER_SIZE` drifted 16 → 12 in the untracked `backend/.env` since the last sync**, and `PositionTiers` derives every tier cutoff from it (qb1 = top 16 then, top 12 now). Re-tiering the live board with the projections *frozen* reassigns **98/671** labels — Patrick Mahomes qb1→qb2 at an unchanged 284.18. So a re-sync today carries three independent changes, not one, and only two of them are H-phase work. **The decomposition** (LIVE = the Mahomes board in Mongo, synced from the *07-17* blend — not 07-28, a second thing the row assumed): config drift alone → 0 projections move, 98 tiers move; data refresh 07-17→07-28 alone → 3/200 top-200 projections move (Kittle +6.40, Lamar +3.00, Love −2.00), 1 tier; **H2 alone → 200/200 top-200 move, mean |Δ| 7.13, 6 tiers**; **ffanalytics alone → 199/200 move, mean |Δ| 3.92, 19 tiers**. `position_max_points` vs LIVE: H2 gives k +23.87, rb +22.69, wr +3.66, te +3.12, dst +2.33, qb **−4.39**; +ffanalytics gives k +25.18, rb +27.07, wr +6.81, te −2.66, dst −0.87, qb −10.68. **`value_over_wait` — the thing the user actually sees — barely moves.** Seed-paired over 3 live Mahomes leagues × 5 seeds at pick 1, and over rounds 3/5/7/9/11 × 3 seeds on a scripted identical draft: **zero** recommendation flips everywhere except round 7, a genuine TE/QB near-tie the engine itself flags "either is defensible" (LIVE te 11.92 vs qb 8.90; with ffanalytics 7.98 vs 7.97). That flip appears under the **plain data refresh too**, so it is not attributable to H2 or ffanalytics — it is a coin-flip decision that the RNG owns. **H2 gate: ADOPT.** It fixes two measured defects (James Conner 29.90 → 68.15, the espn-0.0 sentinel that was halving a real projection; the kicker scale disagreement) and costs 23 more dropped records (672→649, still 4x the 150 picks a 10-team draft consumes). **ffanalytics gate: ADOPT.** 1870/1902 rows resolve against the ESPN anchor; it **adds 61** projected players (649→710), more than repaying H2's 23 drops; its rescale factors are the *best-behaved of any source* (qb 0.948, rb 1.007, wr 0.970, te 0.874, dst 0.854, k 1.007 — compare sleeper's k 1.412), so the blend is not straining to absorb it; and it gives **465 players a third projection source**, lifting `projection_spread` coverage 478→585. That third source is the precondition H2's docstring and H6's note both named for real outlier handling — it now exists. **Caveat worth carrying:** ffanalytics runs systematically low on rookies and second-year players (Jeremiyah Love −13.4, Jadarian Price −11.7, Matthew Golden −11.3, Carnell Tate −9.6, KC Concepcion −10.8, Denzel Boston −9.9 all fall). Its biggest single mover, Stefon Diggs 174.19→141.88, is a *coverage* gain not a scale artifact: Diggs had **one** source before (sleeper 170.2, spread `None`) and ffanalytics's 113.0 is the first second opinion he has had. **WHAT IS BLOCKED ON THE OWNER (three questions, in `docs/specs/H5_ADOPTION_GATE.md`):** (1) run `POST /league/{id}/player/sync` — authority limits forbid it unasked, and it must be preceded by a `mongodump` because git does not protect Mongo; (2) decide `ROSTER_SIZE` **before** that sync, because the sync bakes today's value into every tier — note 12 matches Skunkweed exactly and 16 matches neither league (Mahomes is 10 teams), so the drift arguably *improved* correctness by accident, but tier cutoffs are a global and no per-league override exists; (3) whether to wire ffanalytics in permanently via `register_push_source("ffanalytics", parse_udk_rows)` — a one-line code change H1 already made possible, but a code change, so it belongs to a follow-on row (H10), not to this measurement row. | 1d | ~~H2, H4~~ **H2 (done) — H4 optional** |
| H6 | **Fallback: native Python projection adapters.** If H3 fails, skip R entirely — write `BaseSourceAdapter` subclasses for NumberFire / CBS / FFToday. Less coverage than ffanalytics, zero new runtime, perfect fit for the existing framework. | [Sonnet] | **❌ CLOSED (2026-07-31) — H3 passed, this is the arm that did NOT get taken.** H6 was the fallback, mutually exclusive with H4/H5, only eligible if H3 failed or blew its kill-switch. It passed cleanly in ~25 minutes instead, so the ffanalytics arm (H4/H5) is now the live path and this row is off the table unless H4 later hits a wall the pts_bracket-workaround doesn't cover. Left the rest of the original note below for history. Worth more after H2 than the row implies: a THIRD projection source is what makes real outlier handling possible for the first time — if this ever reopens, that reasoning still holds. A session skipped the unrun `[human]` H3 and routed here on 2026-07-31 before this decision existed; see the selection rule in [Session Protocol](#session-protocol). | 1.5d | ~~H1 + H3-failed~~ **not eligible — H3 passed** |
| H7 | **FantasyPros expert-disagreement signal.** Extend the existing adapter to capture std-dev across experts → an uncertainty field feeding tier confidence and the near-tie margin. | [Opus 5] → [GLM 5.2] | **✅ Done (2026-08-02), branch `h7-expert-disagreement`, commit `56d1807`. 714 tests passing** (706 + 8). Ringer-routed, GLM 5.2, **passed first try** — 51,136 tokens, 160s, despite code-feature being GLM's probation lane. **The signal was already arriving and being thrown away:** the FantasyPros payload the adapter fetches carries `rank_std` (dispersion across ~130 expert rankers) for **508 of 508** players, plus `rank_min`/`rank_max`/`rank_ave`, and `_parse_players` read only `rank_ecr`/`pos_rank`/`tier`. Measured distribution: min 0.5, median 25.44, p75 36.45, max 112.5; top-200 median 12.23. Now captured as `expert_rank_std` on `SourceRecord` → `SourceRankingRecord` → `BlendedRankingRecord` (note `base.py`'s `fetch_batch` enumerates fields explicitly, so it needed the mapping too or the value would have died in the adapter). Both access paths share `_parse_players`, so API and page embed are both covered. **The design question the row posed is settled, with evidence.** The two channels do NOT combine raw — `rank_std` is a spread over ordinal ranks, `projection_spread` is in points. But they agree on *who* is uncertain: the most-disputed top-200 players by expert std (Diggs 52.36, Conner 48.70, Concepcion 47.33, Deebo 43.68, Brissett 41.32) are almost exactly H5's biggest ffanalytics movers. So each channel is converted to a **percentile rank within the blend's own population** (self-calibrating, no constants to go stale) and `tier_confidence` takes the **MAXIMUM** of the available percentiles — deliberately not the mean, because either channel screaming disagreement should be enough to distrust the number, and averaging lets a confident channel mask a loud one. Thresholds: low ≥ 0.75, medium ≥ 0.40, else high; None when neither channel exists. Diggs lands at 0.913 and Conner at 0.886, so both come out low. Distribution over the 769-record blend: 157 high / 228 medium / 218 low / 210 None. **A LIMITATION THE MEASUREMENT EXPOSED — carry it into H12.** The `max` rule is dominated by the expert channel for **kickers**, where high rank dispersion reflects expert *indifference* rather than real uncertainty about points: the six lowest-confidence players in the top 200 are all kickers, e.g. Ryan Fitzgerald with `expert_rank_std` 90.2 but a `projection_spread` of only 5.71. At skill positions the signal is clean — 38 of 166 top-200 skill players are low, and the list reads correctly (Diggs, Aiyuk, Nailor, Concepcion, Rodgers, Dulcich, Brissett, Mendoza: aging vets, injury returns, rookies, QB competitions). This does no harm today because nothing consumes the field yet; **H12 must scope its consumer to skill positions**, which is free — `SCARCITY_POSITIONS` is already qb/rb/wr/te only. **No consumer was wired, deliberately.** Making the near-tie margin uncertainty-aware changes live draft recommendations, and this project requires that to be measured before adoption — that is H12, not a quiet add-on to an adapter row. `blend.py`'s H2 contract docstring was appended to, not rewritten, and `blended_value` / `blended_projection` are asserted unchanged. | 1d | — |
| H8 | **Preseason SOS.** *Optional, defer.* The only gap C2/C5 don't cover. Must land in Mongo via ingestion — see the structural cached-only blocker above. | [Sonnet] | **✅ Done (2026-08-04), branch `h8-preseason-sos`. 744 tests passing** (733 + 11 new). **Implemented directly rather than via Ringer** — a small, incremental extension of an already-established pattern (C2's methodology, H1's push-source-style ingestion split), matching H1's own precedent for when delegating costs more than it saves. **Source chosen: nflverse's real per-game stats, not a paid feed.** Fantasy Nerds stayed dropped (paid, per the preamble); instead of buying anything, this reuses the free CSV C4 already established a pattern for (`data_sources/nflverse.py`), but computes points-allowed-by-defense directly from the whole league's real games (`opponent_team` + `fantasy_points_ppr` per player-week) rather than C2's roster-subsample — a cleaner, larger sample than C2 gets, just for a season that's already over. **A real, unplanned defect fell out of sourcing this:** nflverse's `player_stats_{season}.csv` release (the one C4's `NflverseUsageAdapter` still reads) was deprecated 2025-08-01 in favor of `stats_player_week_{season}.csv` — verified live, `player_stats_2025.csv` 404s while the new release serves real 2025 data. This module uses the current release (`PLAYER_STATS_WEEK_URL` in `data_sources/preseason_sos.py`); **C4 itself was left untouched** (out of scope for this row) but will silently lose target/carry enrichment for 2025+ once its cached 2024-and-earlier files age out — worth a follow-up row. **New: `models/preseason_sos.py`** (cached-only read, no `data_sources` import — a new test asserts it directly, alongside the existing `test_cached_only_modules_never_import_data_sources` guard, which still passes unmodified) holds `PreseasonDefenseStrength` (one row per target_season/position/defense) and the read helpers; **`data_sources/preseason_sos.py`** holds the nflverse adapter and `ingest_preseason_sos()`. **The weak-signal call, made explicit rather than hidden:** a full prior season is a reliable measurement of *last* year's defense, but rosters/coordinators turn over before the season being drafted, so the observed ratio is damped toward neutral by `PRESEASON_SOS_DAMPING` (new config constant, default 0.5 — a defense that allowed 40% more than average carries forward as +20%) before the same `MULTIPLIER_CLAMP` C2 uses. **Wired as two endpoints**, following the `/rankings/refresh` + `/rankings/blended` shape rather than a draft-specific route (the table is season-wide, not per-draft): `POST /rankings/preseason_sos/refresh?target_season=&source_season=` ingests (defaults `source_season` to `target_season - 1`); `GET /rankings/preseason_sos?target_season=&position=&defense=` reads the cached table, defaulting to neutral (multiplier 1.0) when nothing has been ingested yet — same shape as C2's `strength_for()`. **Verified against real data, not just fixtures:** ran both endpoints against the live backend — `refresh?target_season=2026&source_season=2025` pulled real 2025 nflverse data (160 rows: 32 defenses × QB/RB/WR/TE/K, 0 for DST since nflverse's player-level file has no team defense rows) in one call, and the read endpoint returned plausible ranked multipliers (e.g. RB: CIN softest at 1.1424, sample rank 1 of 32, each on 17 real games). **No consumer wired** — like H12's `tier_confidence`, this stores the signal but does not plug it into `value_over_wait` or any draft-time recommendation; per this project's own measure-before-adopting philosophy, and because the row itself calls the underlying signal weak, wiring a consumer without measuring its effect on real recommendations would be the same mistake H5 was written to catch. **A live `preseason_defense_strength` collection for target_season 2026 now exists in the real dev Mongo** from endpoint verification (real, useful data — not a fixture artifact — so left in place); no league/player document was touched. | 1d | — |
| ~~H9~~ | ~~Fantasy Nerds integration~~ | — | **Dropped** — redundant and paid. See above. | — | — |
| H10 | **Wire ffanalytics in as a permanent push source.** `register_push_source("ffanalytics", parse_udk_rows)` in `backend/data_sources/service.py`, plus a round-trip test through the generic `POST /rankings/push/ffanalytics` route. | [Sonnet] → [GLM 5.2] | **✅ Done (2026-08-02), branch `h10-register-ffanalytics`, commit `e1ba33c`. 706 tests passing** (702 + 4). Ringer-routed, GLM 5.2, **passed first try** — 21,415 tokens, 47 seconds, the cheapest row in the phase, because H1 and H4 had already done the hard parts: H1 made `PUSH_SOURCE_PARSERS` a registry and the route generic over it, and H4 deliberately spelled the R producer's CSV headers to match `udk.py`'s `COLUMN_ALIASES`, so ffanalytics reuses `parse_udk_rows` **verbatim** — the same function object, not a copy, which one of the new tests asserts directly. Six lines in `service.py` (five of them the comment explaining the reuse) and four tests in `test_phase2_flow.py`: registered out of the box with no monkeypatching, parser identity, a real upload through the route whose arithmetic is asserted rather than its vocabulary, and an unregistered name still 404ing so registering one source did not make the route permissive. **This writes nothing to Mongo and changes no existing blend.** `all_sources()` now includes ffanalytics, so `rebuild_blend()` will look for a stored ffanalytics batch — but none is stored, so every blend is byte-identical until someone actually uploads a CSV. Adoption was gated on H5's measurement and signed off by the repo owner on 2026-08-02. **Operational note:** the producer needs `R_LIBS_USER=~/R/library` under WSL or `library(ffanalytics)` fails outright; the CSV it writes is a regenerable artifact and is gitignored. | 0.25d | **H5 measured — owner sign-off (given 2026-08-02)** |
| H11 | **Per-league tier cutoffs — kill the `ROSTER_SIZE` ambiguity.** `PositionTiers` derived every `position_tier` boundary from the module-global `config.ROSTER_SIZE`, read from the environment at import. Now derived from `len(league.teams)` at sync time. | [Sonnet] → [GLM 5.2] | **✅ Done (2026-08-02), branch `h11-per-league-tier-cutoffs`, commit `fb9ebf5`. 702 tests passing** (690 + 12 new in `backend/tests/test_position_tiers.py`). Ringer-routed, GLM 5.2 via OpenCode, **passed on attempt 2** (102,490 tokens, 333s); attempt 1 was rejected by the ownership gate for leaving two scratch probe files in the repo, and the retry cleaned them up on its own. `PositionTiers.for_team_count(n)` recomputes every cutoff from a team count using the same formulas, falling back to `ROSTER_SIZE` on a None/non-positive count so a bad value can never zero the cutoffs (which would drop the whole pool into tier 3); the class defaults are untouched, so every un-migrated caller and every stored document behaves exactly as before. `Players` gained an optional `team_count` (None default — it is an `EmbeddedModel` persisted inside `League`, so stored docs had to keep loading), and `app.py` threads `len(league.teams)` through all three from-scratch construction sites: the CSV upload, the blend sync, and the historical-distribution build. **A real bug fell out that the row never specified:** the validator mutated the caller's `Player` objects in place, so two `Players` built from the SAME list corrupted each other — the second build overwrote the first's labels. The behavior check caught it (it builds two boards from one shared list on purpose); the fix is a `model_copy()` on ingest. **Measured, projections frozen:** Mahomes (10 teams) now tiers to exactly 10 qb1s and Skunkweed (12) to 12 — the same players, different boards, which one global could never express. Re-tiering the live Mahomes board under its own count moves **147** `position_tier` labels vs the stored ones, *more* than the 98 H5 measured under the global, because 16 → 10 is a bigger jump than 16 → 12. `backend/scripts/h5_adoption_gate.py` clause 2 was **re-measured, not relaxed** — its floor moved 90 → 140 and it now asks the honest question (what a sync will actually produce, i.e. this league's own count) rather than what the global would do; a new clause asserts the qb1 count equals the team count. **Deliberately out of scope:** the `Players(players=filtered)` in the GET players route still re-tiers a filtered subset — a pre-existing oddity, left alone and not made worse. `League.roster_size` (players per team, read by nobody) was left in place; renaming a persisted field needs a Mongo migration and buys nothing, so `PositionTiers`' docstring now states the distinction instead. **No Mongo writes** — this changes what a *future* sync produces. | 0.5d | **— (do before the H5 sync)** |
| H12 | **Give `tier_confidence` a consumer: an uncertainty-aware near-tie margin.** `value_over_wait.TIE_MARGIN_POINTS` was a flat 5.0; it now widens when the two positions in contention are led by low-confidence players. | [Opus 5] → [GLM 5.2] | **✅ Built and MEASURED (2026-08-02), branch `h12-uncertain-tie-margin`, commit `606e77a`. 732 tests passing** (714 + 18). Ringer-routed, GLM 5.2, **passed first try** — 34,129 tokens, 125s. `UNCERTAINTY_TIE_WIDENING` (module constant, **defaults to 0.0 = exactly today's behaviour**) and `effective_tie_margin(a, b, pools, year)`, which reads `tier_confidence` off the best undrafted player each side would actually hand you, scores low/medium/high as 1.0/0.5/0.0, and returns `TIE_MARGIN_POINTS * (1 + knob * mean_score)`. `Player` gained `tier_confidence` and the blend sync now carries it, without which the whole thing would have been inert on real boards. **k and dst never widen**, per H7's measurement that expert dispersion there is indifference rather than uncertainty. `_build_reason` takes optional `pools` and falls back to the flat constant when absent, so every existing caller is unchanged. **THE MEASURED GATE.** The live boards carry no `tier_confidence` (synced pre-H7), so measuring on them would have shown a trivial nil result; the board was instead rebuilt in memory as a post-H5/H7/H10 sync would produce it — 710 players, 166 high / 231 medium / 217 low / 96 None — and swept seed-paired over knob ∈ {0, 0.5, 1, 2} × rounds 1/3/5/7/9/11 × 5 seeds (30 decisions per setting). **Result: ZERO position flips and ZERO pick flips at every setting.** The recommendation itself never moves. What changes is only the hedge: tie-flag changes 7/30 at 0.5, 9/30 at 1.0, 10/30 at 2.0, **every one of them in the same direction** — off → on, i.e. the engine volunteers "either is defensible" where it previously asserted a single answer. They cluster at rounds 5 and 7, the same near-ties H5 found. That is the safest shape this change could have taken: it cannot send you to the wrong position, it can only tell you the choice is closer than it looks. **Recommended default 0.5** (margin 5 → 6): 1.0 is defensible; **2.0 is too loose** — it starts calling an 8.7-point gap a tie ("QB 26.8 and TE 18.1 are within 10 pts"). **Shipped at 0.0 pending the owner's call**, because changing what the draft table says is not a session's decision even when the measurement is favourable. Flipping it is a one-line change. | 0.5d | **H7 (done)** |
| H13 | **ADOPT: run the sync.** Rebuild the blend with ffanalytics and materialize it into the master leagues, adopting H2, the third projection source, and per-league tier cutoffs in one authorised operation. | [Opus 5] | **✅ Done (2026-08-02), branch `h13-adoption-sync`. Owner-authorised in session. 733 tests passing.** Done by hand, not Ringer-routed — it is a data operation against live Mongo, not a code change, and the Session Protocol reserves it for an explicit ask. **Snapshot first:** `mongodump` is unavailable (the MongoDB Database Tools are not installed on this machine), so `backend/scripts/mongo_snapshot.py` was written as the substitute CLAUDE.md's data-safety rule requires — a full `bson.json_util` dump of all 17 collections that re-reads and checksums itself and compares counts against live before reporting success, with a `restore` path. Verified snapshot at `C:\fantasy-football-backups\20260802T162037Z` (13 league docs, 7524 historical picks). **Sequence:** `POST /rankings/push/ffanalytics` with the R producer's 1902-row CSV → 1870 resolved, blend rebuilt to 803 records / 710 projected across five sources; then `POST /league/{id}/player/sync` on the **two master leagues only**. **The nine mid-draft copies were deliberately NOT synced** — sync replaces the player pool and would wipe their in-progress picks (one is 28 picks deep); copies are made fresh from a master when a draft starts, which is exactly what the sync endpoint's own docstring assumes. Verified after the fact: all 11 copies byte-identical to the snapshot, picks intact. **Every number H5 predicted before the sync was reproduced by it:** 671 → 710 players (16 dropped, 55 added), 643/655 projections moved, `position_max_points` qb 365.38→354.70, rb 338.68→365.75, wr 334.48→341.29, te 246.27→243.61, dst 116.02→115.15, k 143.78→168.96 — all matching board C to the cent, as did Bijan Robinson 365.75, Stefon Diggs 141.88 and Brandon Aubrey 168.96. **H11 is visible in production:** Mahomes now carries 10 qb1s and Skunkweed 12, on an identical player pool — impossible before this phase. Tier reassignments 150 (Mahomes) and 103 (Skunkweed). **One prediction was wrong and it was mine, not the engine's:** the guard asserted James Conner at 68.15, which was his H2 *two-source* value; with ffanalytics he is 75.78 (espn 0.0 sentinel dropped, sleeper 59.8 rescaled to 68.2, ffanalytics 82.8 rescaled to ~83.4). Corrected with the arithmetic recorded. **`tier_confidence` is derived from the projection-spread channel only** (585 records) — the stored fantasypros batch predates H7 so it carries no `expert_rank_std`; a normal `POST /rankings/refresh` will populate the expert channel. A full refresh was deliberately NOT run here, because it would have introduced unmeasured data drift on top of an adoption that had been measured precisely. **H12 adopted at 0.5** in the same branch. **`backend/scripts/h5_adoption_gate.py` flipped from gate to guard:** it was built to fail the moment a sync ran, it did exactly that, and it is now rewritten to assert the adopted state — including that no draft copy has been re-synced, which is the clause with real teeth. | — | **H5 + H11 + owner authorisation** |
| H13b | **Refresh the sources so H7's expert channel goes live, then re-adopt.** | [Opus 5] | **✅ Done (2026-08-02), branch `h13b-refresh-adoption`. Owner-directed.** H13 adopted a board whose `tier_confidence` ran on ONE channel: the stored fantasypros batch predated H7, so `expert_rank_std` coverage was 0 and only `projection_spread` fed the score. `POST /rankings/refresh` (all four pull sources succeeded) fixed that — **expert coverage 0 → 508**, and `tier_confidence` moved from 233 high / 205 medium / 147 low to **157 / 228 / 239**, i.e. much closer to the distribution H12's gate was actually measured against (166 / 231 / 217). **Drift measured before re-adopting**, as usual: 685/709 projections moved but mean \|Δ\| only 1.58 (median 0.43), top-200 mean 2.12 — a routine five-day data refresh, not a recalibration. **The one outlier was checked rather than waved through:** Ricky Pearsall −62.05, which turned out to be corroborated by two independent sources (espn 165.35 → 58.97, sleeper dropped him entirely), so it is news, not corruption — as were Deebo Samuel +21.32 (espn began projecting him) and Malik Nabers +11.19. Masters re-synced; the mid-draft copies again untouched. **The guard script needed a second, more important fix.** Its post-H13 version pinned the exact projections H5 predicted, and they went stale within the hour on this very refresh — a guard that fails on correct behaviour is worse than none. It now asserts STRUCTURAL invariants that survive any refresh: the board equals the latest blend, cutoffs follow each league's team count, no projection is non-positive, the kicker ceiling stays above its unrescaled pre-H2 143.78, and Conner stays clear of the 29.90 sentinel. Snapshot before the refresh: `C:\fantasy-football-backups\20260802T163517Z`. 733 tests passing. | — | **H13** |
...
8. **`ROSTER_SIZE` drift, found by H5 (2026-07-31) — decide before any
   re-sync.** The live leagues' `position_tier` labels were assigned under
   `ROSTER_SIZE=16`; today's untracked `backend/.env` sets nothing, so the
   code default 12 applies. `PositionTiers` derives every cutoff from it,
   ...
   than from an env-read global — **shipped as H11 on 2026-08-02**, so the
   sync is no longer blocked on this. Note the churn a sync now carries is
   **147** labels, not 98: Mahomes is a 10-team league and its stored
   labels were cut at 16. Reproduce with
